...
# GPUs: 1 (Qwen3-4B fits on a single XPU device)
...
# oneCCL/OFI settings required for XPU TP>1 on Intel XPU.
# Without these, allreduce hangs in ofi_dynpoll_wait_epoll because:
#   - CCL defaults to ATL/MPI but no MPI launcher is present
#   - OFI auto-selects the tcp provider for intra-node, which deadlocks
#   - shm provider must be enabled at BOTH the CCL layer (CCL_ATL_SHM=1)
#     AND implicitly via OFI (CCL picks shm once allowed)
